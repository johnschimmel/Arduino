...
public class Editor extends JFrame implements RunnerListener {
	...
	static final int SHORTCUT_KEY_MASK = Toolkit.getDefaultToolkit()
			.getMenuShortcutKeyMask();
	/** Command-W on Mac OS X, Ctrl-W on Windows and Linux */
	static final KeyStroke WINDOW_CLOSE_KEYSTROKE = KeyStroke.getKeyStroke('W',
			SHORTCUT_KEY_MASK);
		...
		public boolean importData(JComponent src, Transferable transferable) {
			...
			try {
				...
				if (transferable
					...
					// Some platforms (Mac OS X and Linux, when this began)
					// preferred
	...
	protected void applyPreferences() {
		...
		textarea.setDisplayLineNumbers(Preferences
				.getBoolean("editor.linenumbers"));
		...
		for (int i = 0; i < folders.length; i++) {
			...
			try {
				...
				for (int j = 0; j < archives.length; j++) {
					className = findClassInZipFile(folders[i].getName(),
							archives[j]);
					if (className != null)
						break;
				}
				...
				/*
				 * // Alternatively, could use manifest files with special
				 * attributes: //
				 * http://java.sun.com/j2se/1.3/docs/guide/jar/jar.html //
				 * Example code for loading from a manifest file: //
				 * http://forums.sun.com/thread.jspa?messageID=3791501 File
				 * infoFile = new File(toolDirectory, "tool.txt"); if
				 * (!infoFile.exists()) continue;
				 * 
				 * String[] info = PApplet.loadStrings(infoFile); //Main-Class:
				 * org.poo.shoe.AwesomerTool //String className =
				 * folders[i].getName(); String className = null; for (int k =
				 * 0; k < info.length; k++) { if (info[k].startsWith(";"))
				 * continue;
				 * 
				 * String[] pieces = PApplet.splitTokens(info[k], ": "); if
				 * (pieces.length == 2) { if (pieces[0].equals("Main-Class")) {
				 * className = pieces[1]; } } }
				 */
	...
	class SerialMenuListener implements ActionListener {
		...
		/*
		 * public void actionPerformed(ActionEvent e) {
		 * System.out.println(e.getSource()); String name =
		 * e.getActionCommand(); PdeBase.properties.put("serial.port", name);
		 * System.out.println("set to " + get("serial.port"));
		 * //editor.skOpen(path + File.separator + name, name); // need to push
		 * "serial.port" into PdeBase.properties }
		 */
	}
			...
			for (Enumeration enumeration = CommPortIdentifier
				...
				if (commportidentifier.getPortType() == CommPortIdentifier.PORT_SERIAL) {
					...
					rbMenuItem = new JCheckBoxMenuItem(curr_port,
							curr_port.equals(Preferences.get("serial.port")));
	...
	protected JMenu buildHelpMenu() {
		...
		/*
		 * // testing internal web server to serve up docs from a zip file item
		 * = new JMenuItem("Web Server Test"); item.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * //WebServer ws = new WebServer(); SwingUtilities.invokeLater(new
		 * Runnable() { public void run() { try { int port = WebServer.launch(
		 * "/Users/fry/coconut/processing/build/shared/reference.zip");
		...
		/*
		 * item = new JMenuItem("Browser Test"); item.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * //Base.openURL("http://processing.org/learning/gettingstarted/");
		 * //JFrame browserFrame = new JFrame("Browser"); BrowserStartup bs =
		 * new BrowserStartup(
		 * "jar:file:/Users/fry/coconut/processing/build/shared/reference.zip!/reference/setup_.html"
		 * ); bs.initUI(); bs.launch(); } }); menu.add(item);
		 */
	...
	/**
	 * Same as newJMenuItem(), but adds the ALT (on Linux and Windows) or OPTION
	 * (on Mac OS X) key as a modifier.
	 */
	static public JMenuItem newJMenuItemAlt(String title, int what) {
		JMenuItem menuItem = new JMenuItem(title);
		// int modifiers = Toolkit.getDefaultToolkit().getMenuShortcutKeyMask();
		// menuItem.setAccelerator(KeyStroke.getKeyStroke(what, modifiers));
		menuItem.setAccelerator(KeyStroke.getKeyStroke(what,
				SHORTCUT_ALT_KEY_MASK));
		return menuItem;
	}
	 ...
	 * found in this class. This will maintain compatibility with future
	 * releases, which will not use JEditTextArea.
	 */
	 ...
	 * this will be the last character actually selected, no matter the
	 * direction of the selection. That is, if the user clicks and drags to
	 * select lines 7 up to 4, then the caret position will be somewhere on line
	 * four.
	 */
	...
	protected void setCode(SketchCode code) {
		...
		textarea.setDocument(document, code.getSelectionStart(),
				code.getSelectionStop(), code.getScrollPosition());
	...
	protected void handleCommentUncomment() {
		...
		for (int i = startLine; commented && (i <= stopLine); i++) {
			...
			if (pos + 2 > length) {
				...
				// Check the first two characters to see if it's already a
				// comment.
		...
		try {
			...
			while (!tmp.matches(regexp)) {
				...
				if (current - startOffset < 0) {
					...
					tmp = textarea.getDocument().getText(current - startOffset,
							1);
			}
			startOffset--;

			int length = endIndex - current + startOffset;
			text = textarea.getDocument()
					.getText(current - startOffset, length);
	...
	protected void handleFindReference() {
		...
		if (referenceFile == null) {
			statusNotice(I18n.format(_("No reference available for \"{0}\""),
					text));
		} else {
	...
	class DefaultStopHandler implements Runnable {
		public void run() {
			try {
				// DAM: we should try to kill the compilation or upload process
				// here.
			} catch (Exception e) {
				statusError(e);
			}
	...
	protected boolean checkModified() {
		...
		String prompt = I18n.format(_("Save changes to \"{0}\"?  "),
				sketch.getName());

		if (!Base.isMacOS()) {
			int result = JOptionPane.showConfirmDialog(this, prompt,
					_("Close"), JOptionPane.YES_NO_CANCEL_OPTION,
			...
			// suck--workarounds for the Mac and Apple's snobby attitude about
			// it!
			// I think it's nifty that they treat their developers like dirt.

			// Pane formatting adapted from the quaqua guide
			// http://www.randelshofer.ch/quaqua/guide/joptionpane.html
			JOptionPane pane = new JOptionPane(_("<html> "
			...
			String[] options = new String[] { _("Save"), _("Cancel"),
					_("Don't Save") };
	...
	protected boolean handleOpenInternal(String path) {
		...
		if (pdeName.equals(fileName) || inoName.equals(fileName)) {
			...
			// user selected a .java from the same sketch, but open the .pde
			// instead
			...
			if (result == JOptionPane.YES_OPTION) {
				...
				if (properFolder.exists()) {
					Base.showWarning(_("Error"), I18n.format(
							_("A folder named \"{0}\" already exists. "
									+ "Can't open sketch."), properParent),
							null);
					return false;
				}
				...
				try {
					...
					Base.showWarning(_("Error"),
							_("Could not copy to a proper location."), e);
					return false;
				}
	...
	/**
	 * Actually handle the save command. If 'immediately' is set to false, this
	 * will happen in another thread so that the message area will update and
	 * the save button will stay highlighted while the save is happening. If
	 * 'immediately' is true, then it will happen immediately. This is used
	 * during a quit, because invokeLater() won't run properly while a quit is
	 * happening. This fixes <A
	 * HREF="http://dev.processing.org/bugs/show_bug.cgi?id=276">Bug 276</A>.
	 */
	...
	public boolean serialPrompt() {
		...
		String result = (String) JOptionPane.showInputDialog(this, I18n.format(
				_("Serial port {0} not found.\n"
						+ "Retry the upload with another serial port?"),
				Preferences.get("serial.port")), "Serial port not found",
	...
	/**
	 * Called by Sketch &rarr; Export. Handles calling the export() function on
	 * sketch, and queues all the gui status stuff that comes along with it.
	 * <p/>
	 * Made synchronized to (hopefully) avoid problems of people hitting export
	 * twice, quickly, and horking things up.
	 */
	/**
	 * Handles calling the export() function on sketch, and queues all the gui
	 * status stuff that comes along with it.
	 * 
	 * Made synchronized to (hopefully) avoid problems of people hitting export
	 * twice, quickly, and horking things up.
	 */
	...
	/**
	 * Checks to see if the sketch has been modified, and if so, asks the user
	 * to save the sketch or cancel the export. This prevents issues where an
	 * incomplete version of the sketch would be exported, and is a fix for <A
	 * HREF="http://dev.processing.org/bugs/show_bug.cgi?id=157">Bug 157</A>
	 */
	protected boolean handleExportCheckModified() {
		...
		int result = JOptionPane.showOptionDialog(this,
				_("Save changes before export?"), _("Save"),
			...
			if (re.hasCodeLine()) {
				...
				if (line >= textarea.getLineCount()) {
					...
					if (textarea.getLineText(line).length() == 0) {
						// The last line may be zero length, meaning nothing to
						// select.
				...
				if (line < 0 || line >= textarea.getLineCount()) {
					System.err.println(I18n.format(_("Bad error line: {0}"),
							line));
				} else {
